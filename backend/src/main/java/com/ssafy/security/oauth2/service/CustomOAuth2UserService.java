...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
    ...
    public OAuth2User loadUser(OAuth2UserRequest oAuth2UserRequest) throws OAuth2AuthenticationException {
        log.info("소셜로부터 받은 userRequest 데이터 확인: '{}'", oAuth2UserRequest);
    ...
    private OAuth2User processOAuth2User(OAuth2UserRequest oAuth2UserRequest, OAuth2User oAuth2User) {
        ...
        //소셜로부터 받아온 request 정보
        log.info("oAuth2UserRequest.getClientRegistration : '{}'", oAuth2UserRequest.getClientRegistration());
        log.info("oAuth2UserRequest.getAccessToken : '{}'", oAuth2UserRequest.getAccessToken());
        log.info("oAuth2UserRequest.getAdditionalParameters : '{}'", oAuth2UserRequest.getAdditionalParameters());
        ...
        if (userOptional.isPresent()) {
            ...
            user = updateExistingUser(user, oAuth2UserInfo); //소셜로그인으로 받아온 닉네임정보 바뀐 것이 있다면 바꿔주기
        }else { //아닌 경우
            log.info("우리회원x-회원가입 필요");

            //소셜로부터 받아온 회원 정보만을 return
            user = new User();
            user.setUserId(oAuth2UserInfo.getUserId()); //사용자 이메일
            user.setNickname(oAuth2UserInfo.getNickname()); //사용자 이름
            user.setType(ProviderType.valueOf(oAuth2UserRequest.getClientRegistration().getRegistrationId().toUpperCase())); //타입 지정
        }
